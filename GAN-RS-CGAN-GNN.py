...
# 尝试导入 CausalML 的 XLearner (作为对比验证)
HAS_CAUSALML = False
try:
    from causalml_source_local.inference.meta.xlearner import XLearner
    HAS_CAUSALML = True
except ImportError:
    try:
    ...
    def __init__(self, num_edge_types, num_channels, w_in, w_out, num_nodes, num_layers=2):
        ...
        for i in range(num_layers):
            if i == 0:
                ...
                self.layers.append(GTLayerDense(num_edge_types, num_channels, num_nodes, first=False))
...
def build_enhanced_adjacency(X_clinical, X_genes, T_vec=None, threshold=0.5):
    """
    构建多视图邻接矩阵，用于 GTN。
    包含：
    1. 临床特征相似性图
    2. 基因表达相关性图
    3. 倾向性得分 (Propensity) 相似性图 (如果提供了 T)
    4. 恒等图 (Identity)
    """
    # 1. 临床特征相似性 (Clinical Similarity)
    ...
    # 2. 基因相关性 (Gene Correlation - Patient Similarity)
    # 计算样本间的 Pearson 相关系数
    dist_gene = cdist(X_genes, X_genes, metric='correlation')
    sim_gene = 1 - dist_gene
    adj_gene = (sim_gene > threshold).astype(np.float32)
    
    # 3. 倾向性得分相似性 (Propensity Similarity)
    # 如果有治疗变量 T，计算倾向性得分并连接 PS 相近的样本
    if T_vec is not None:
        # 使用简单的 Logistic 回归估算 PS
        lr = LogisticRegression(solver='liblinear')
        lr.fit(X_clin_norm, T_vec)
        ps = lr.predict_proba(X_clin_norm)[:, 1].reshape(-1, 1)
        dist_ps = cdist(ps, ps, metric='euclidean')
        # PS 差异越小，相似度越高
        sim_ps = np.exp(-dist_ps ** 2 / 0.1) # 0.1 为带宽
        adj_ps = (sim_ps > threshold).astype(np.float32)
    else:
        # 如果没有 T (在预训练阶段)，使用全 0 或单位阵
        adj_ps = np.eye(len(X_clinical), dtype=np.float32)
        
    # 4. 恒等图 (Identity)
    adj_eye = np.eye(len(X_clinical), dtype=np.float32)
    
    # 堆叠: (4, N, N)
    adjs = np.stack([adj_clin, adj_gene, adj_ps, adj_eye], axis=0)
...
class GTNDragonNet(nn.Module):
    def __init__(self, num_nodes, input_dim, neurons_per_layer=200):
        ...
        # GTN 特征提取器
        # num_edge_types=4 (Clin, Gene, Propensity, Eye), num_channels=2 (学习 2 个元图)
        self.gtn = GTN(num_edge_types=4, num_channels=2, w_in=input_dim, w_out=64, num_nodes=num_nodes, num_layers=2)
        
        # DragonNet Heads
        # 输入维度是 GTN 输出维度: num_channels * w_out = 2 * 64 = 128
        self.repr_dim = 2 * 64
...
def estimate_ate_with_gtn(netG, X_real, Y_real, X_clinical, gene_cols):
    ...
    logger.info(f"使用 GTN-DragonNet 和 XLearner 评估 {len(candidates)} 个基因...")
    ...
    for gene in candidates:
        ...
        # 构建图结构 (包含倾向性得分信息)
        # 注意: 这里对每个基因都重新构建图，因为 PS 依赖于 T
        adj_matrix = build_enhanced_adjacency(W, X_comb, T_vec=T)
        ...
        try:
            ...
            with torch.no_grad():
                ...
                ate_gtn = (y1_pred - y0_pred).mean().item()
            
            # 2. 使用 XLearner (CausalML) 进行验证
            ate_xl = np.nan
            if HAS_CAUSALML:
                # 使用简单的基学习器
                xl = XLearner(models=GradientBoostingRegressor(n_estimators=50),
                              propensity_model=GradientBoostingClassifier(n_estimators=50))
                # 下采样以加速 (如果数据量大)
                if len(W) > 2000:
                    idx_sub = np.random.choice(len(W), 2000, replace=False)
                    xl.fit(X=W[idx_sub], treatment=T[idx_sub], y=Y[idx_sub])
                    cate_xl = xl.predict(X=W[idx_sub])
                else:
                    xl.fit(X=W, treatment=T, y=Y)
                    cate_xl = xl.predict(X=W)
            ...
            results.append({
                'SNP_Site': gene, 
                'ATE_GTN': ate_gtn, 
                'Abs_ATE_GTN': abs(ate_gtn),
                'ATE_XLearner': ate_xl
            })
            
            logger.info(f"Gene {gene}: GTN ATE={ate_gtn:.4f}, XLearner ATE={ate_xl:.4f}")
            
        except Exception as e:
            logger.error(f"GTN/XLearner failed for {gene}: {e}")
            
    res_df = pd.DataFrame(results)
    if not res_df.empty:
        res_df = res_df.sort_values('Abs_ATE_GTN', ascending=False)
